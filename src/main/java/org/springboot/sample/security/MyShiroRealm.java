...
import org.springboot.sample.service.PermissionService;
import org.springboot.sample.service.RoleService;
...
public class MyShiroRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection collection) {
		...
		String loginName = (String) super.getAvailablePrincipal(collection);
		...
		if (user != null) {
            ...
            //用户的角色集合
			info.setRoles(roleService.getRolesNameByUser(user.getId()));
            //用户的角色对应的所有权限，如果只使用角色定义访问权限，下面的四行可以不要
			List<Role> roles = roleService.findListByUserId(user.getId());
			for(Role role : roles) {
				//info.addRole(role.getRolename());
				info.addStringPermissions(permissionService.getPermissionNamesByRoleId(role.getId()));
			}
            // 或者按下面这样添加
